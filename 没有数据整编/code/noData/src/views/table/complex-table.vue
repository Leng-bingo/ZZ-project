<template>
  <div class="app-container">
    <div class="filter-container">
      <el-select v-model="value" filterable class="filter-item" placeholder="请选择数据库" @change="selectTable">
        <el-option
      ...
      <br><br>
      <el-input v-model="listQuery.text" placeholder="标题" style="width: 200px;" class="filter-item" clearable @keyup.enter.native="handleFilter" />
      <el-select v-model="listQuery.type" clearable filterable placeholder="类型" class="filter-item" style="width: 180px;margin-left: 10px" @change="selectlabel">
        <el-option
      ...
      <!-- <el-button class="filter-item" style="margin-left: 10px;" type="primary" icon="el-icon-delete" @click="handleFilter1"> -->
      <el-button class="filter-item" style="margin-left: 10px;" type="primary" icon="el-icon-delete" @click="deleteConfirm">
        删除
      </el-button>
    </div>
    ...
    <el-dialog
      ...
      center
    >
      <span>请确认是否全部删除数据库 </span>
      <span style="font-weight: bold">{{ listQuery.table_name }}</span>
      <span> 中关于 </span>
      <span style="font-weight: bold">{{ listQuery.text }}</span>
      <span> 的全部数据</span>
      <span slot="footer" class="dialog-footer">
        <el-button @click="centerDialogVisible = false">取 消</el-button>
        <el-button type="primary" @click="handleFilter1">确 定</el-button>
      </span>
    </el-dialog>
  </div>
</template>
...
import { just_duplicate, save_item1, find_label1, getWeibo_Table_Data, update_really, delete_weibo_data, create_weibo_data, search_create_weibo_data, go_spider, weibo_table, delete_weibotable_data } from '@/api/mydata'
...
export default {
  ...
  methods: {
    getTabelName() {
      weibo_table().then(res => {
        // console.log(res.data[3])
        // this.options1 = res.data[3]
        for (let i = 0; i < res.data[3].length; i++) {
          this.options1.push({
            label: '标签' + i,
            value: res.data[3][i].TABLE_NAME
          })
        }
      })
    },
    ...
    handleFilter1() {
      ...
      if (this.listQuery.table_name !== '' && typeof (this.listQuery.text) !== 'undefined') {
        // console.log(this.listQuery)
        delete_weibotable_data(this.listQuery).then(res => {
          // console.log(res)
          if (res.data === '1') {
            this.$notify({
              title: '成功',
